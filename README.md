...
- If you want to compute a general Fréchet Classifier Distance with activations (e.g., outputs of the last pooling layer) `act1` and `act2` from another classifier, run `activations2distance(act1, act2)`, where `act1` and `act2` are both numpy arrays of an arbitrary shape `[N, d]`.
## Links
